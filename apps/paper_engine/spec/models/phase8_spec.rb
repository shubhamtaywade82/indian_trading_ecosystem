...
RSpec.describe "Phase 8: Broker Virtualization", type: :model do
  let!(:account) { Account.create!(tenant_id: "t1", mode: "paper_dhan", name: "Dhan Paper Account") }
  ...
  it "allows Kite mode to bypass penny stock block with Kite formatting" do
    kite_account = Account.create!(tenant_id: "t1", mode: "paper_kite", name: "Kite Account")
    MarginAccount.create!(
      account_id: kite_account.id, cash_balance: 1_000_000, blocked_margin: 0, available_margin: 1_000_000, mtm_pnl: 0, realized_pnl: 0
    )
    
    # Passes broker rule, might fail RMS if no cash but here it has 1M cash, 100*50 = 5k. Will pass!
    order = PlaceOrder.call(account: kite_account, payload: {
      instrument_id: 'SUZLON_PENNY', side: 'buy', order_type: 'MARKET', product_type: 'CNC', qty: 100, price: 50
    })
    ...
    order_illiquid = PlaceOrder.call(account: kite_account, payload: {
